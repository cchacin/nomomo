...
class CommitsServiceTest implements WithAssertions {
    ...
    void shouldReturnFromLocal(
        ...
        // Given
        when(localRepository.commitBySha(anyString())).thenReturn(Optional.of(new Commit("local", List.of("local.txt"))));
    ...
    void shouldReturnFromGithubWhenLocalIsEmpty(
            ...
            @Mock final Repository gitHubRepository) throws Exception {
        // Given
        when(localRepository.commitBySha(anyString())).thenReturn(Optional.empty());
        when(gitHubRepository.commitBySha(anyString())).thenReturn(Optional.of(new Commit("git", List.of("git.txt"))));
